...
const share_func = async (type,file) => {
    const options = {
        title:"Share Downloaded", 
        message:"Downloaded using YTD-964", url:`file:///${dir}/${type}/${file}`}
    await Share.shareSingle(options).then(r => console.log(r)).catch(err => console.log(err));
}
        ...
        const render_right = props => {
            return (
                    <View style={[styles.row, {backgroundColor:"white"}]}>
                        <Fw5icon.Button backgroundColor="white" name="play" onPress={()=>open_func(type,v,nav)} size={24} style={{marginRight:24}} color="red" /> 
                        <Entypo.Button backgroundColor="white" name="share" onPress={() => share_func(type,v)} size={24} style={{marginRight:20}} color="blue" />
                    </View>
            );
        }
