...
export default function Downloads({navigation}){
    ...
    const open_func = (file) => {
        ...
        if (path.extname(file) === ".mp4"){
            navigation.navigate("view", {file:file})
        }else if (path.extname(file) === ".mp3"){
            console.log("audio!!!")
            var name = path.parse(file).name;
            const directory = `file:///${dir}/${file}`
    ...
    const available = allFiles.length === 0;
    
    const Nocontent = () => {
        return (
            <View style={{justifyContent:"center", alignItems:"center"}}>
                <Ionicon name="md-file-tray-stacked-sharp" size={100} style={{margin:50}} />
                <Title numberofLines={2} ellipsizeMode="middle">Your all downloaded Items will appear here. Currently It is Empty</Title>
            </View>
        );
    };
