...
export default function Downloads({navigation}){
    ...
    const render_right = (i,props) => {
        return (
        <Menu ref={ref => (menuRef[`menu${i}`] = ref)}
         button={<SimpleLine name="options-vertical" onPress={ref=>(menuRef[`menu${i}`].show())} {...props} style={{marginTop:10}} size={20} />}  >
                ...
                <View>
                    <List.Item style={{padding:15}}
                     titleNumberOfLines={2}
                      title={file_name}
                       key={v} left={icon}
                        right={(props) => render_right(i,props)} />
                    <Divider />
                </View>
            )
        }
