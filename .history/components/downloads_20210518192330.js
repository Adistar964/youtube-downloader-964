...
export default function Downloads({navigation}){
    ...
    const open_func = (file,name) => {
        ...
        if (path.extname(file) === ".mp4"){
            ...
            const audio = new Sound(`${dir}/${file}`);
            Alert.alert("Play Audio", "Are you sure you want to play this audio?",
            [{text:"Yes", onPress:audio.play()},{text:"no"}])
    }}
    ...
    const show_files = (v,i) => {
        if (v !== undefined && typeof v === "string"){
            ...
            const call_share = () => share_func(v, file_name);
