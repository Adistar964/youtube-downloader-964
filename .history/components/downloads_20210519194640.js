...
function VideoDownloads({navigation}){
    ...
    const show_files = (v,i,type) => {
        if (v !== undefined && typeof v === "string"){
            ...
            const render_right = (props) => {
                return (
                <Menu ref={ref => (menuVidRef[`menu${i}`] = ref)}
                 button={<SimpleLine name="options-vertical" onPress={ref=>(menuVidRef[`menu${i}`].show())} {...props} style={props.style} size={20} />}  >
    ...
    const auds = allFiles.audios.map((v,i) => show_files(v,i,"ytd-964 audios"));
...
function AudioDownloads({navigation}){
    ...
    const show_files = (v,i) => {
        if (v !== undefined && typeof v === "string"){
            ...
            const share_icon = (props) => <Entypo name="share" onPress={call_share} {...props} />
            const opt_icon = props => <SimpleLine name="options-vertical" onPress={open_menu} {...props} />
            const call_open = () => open_func(v);

            const render_right = (props) => {
                return (
                <Menu ref={ref => (menuAudioRef[`menu${i}`] = ref)}
                 button={<SimpleLine name="options-vertical" onPress={ref=>(menuAudioRef[`menu${i}`].show())} {...props} style={props.style} size={20} />}  >
                    <MenuItem onPress={call_open} > Open </MenuItem>
                    <MenuItem onPress={call_share} > Share </MenuItem>
                </Menu>
                );
    ...
    const auds = allFiles.audios.map(show_files);
    ...
    const Nocontent = () => {
        return (
            <View style={{justifyContent:"center", alignItems:"center"}}>
                <Ionicon name="md-file-tray-stacked-sharp" size={100} style={{margin:50}} />
                <Title style={{margin:20}}>Your all downloaded Audios will appear here. Currently It is Empty.</Title>
            </View>
        );
    };
